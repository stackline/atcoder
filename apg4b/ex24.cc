...
struct Clock {
  ...
  void shift(int diff_second) {
    int current_sec = hour * 60 * 60 + minute * 60 + second;
    // 時間をシフトする
    int shifted = current_sec + diff_second;

    // マイナス秒の場合は 60 * 60 * 24 から減算する
    if (shifted < 0) {
      shifted = (60 * 60 * 24) + shifted;
    }

    // 秒が1日の最大秒数を超える場合は、1日の最大秒数以内に収める
    if (shifted >= 60 * 60 * 24) {
      shifted = shifted % (60 * 60 * 24);
    }

    // 時分秒を計算する
    hour = shifted / (60 * 60);
    shifted = shifted - (hour * 60 * 60);
    minute = shifted / 60;
    second = shifted - (minute * 60);
  }
};
